    ...
    const fetchTransactions = async () => {
      ...
      try {
        ...
        const response = await axios.get(
          `${process.env.NEXT_PUBLIC_API_URL}/all-transaction?per_page=116`, // Ambil semua
          {
        ...
        // Urutkan transaksi: pending dulu, lalu success, lalu cancelled
        sortedTransactions.sort((a, b) => {
          const statusOrder = { pending: 1, success: 2, cancelled: 3 };
          return statusOrder[a.status] - statusOrder[b.status];
        });

        // Set all transactions yang sudah diurutkan
        setAllTransactions(sortedTransactions);
        setTransactions(sortedTransactions.slice(0, itemsPerPage)); // Menampilkan data untuk halaman pertama
      } catch (error) {
