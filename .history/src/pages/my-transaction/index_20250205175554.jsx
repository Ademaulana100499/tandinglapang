...
export async function getServerSideProps({ req, query }) {
  ...
  try {
    ...
    const { role, roleId } = req.cookies;
    console.log("Role:", role);
    console.log("RoleId:", roleId);

    // Filter data sesuai dengan role dan roleId
    const filteredData =
      role === "user" && Array.isArray(res.data.result)
        ? res.data.result.filter((transaction) =>
            transaction.participants.some(
              (participant) => participant.user_id == roleId
            )
          )
        : res.data.result;
    ...
    // Menentukan data transaksi yang telah difilter
    const transactions = filteredData || [];

    // Memeriksa dan mengambil data pagination
    const currentPage = res.data?.current_page || 1; // Mengakses current_page dari res.data
    const lastPage = res.data?.last_page || 1; // Mengakses last_page dari res.data
