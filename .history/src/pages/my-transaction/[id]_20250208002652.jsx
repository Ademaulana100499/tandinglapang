...
const DetailMyTransaction = ({ data, role }) => {
  ...
  const { handleImageClick, closeModal, fetchData } = useTransactionDetail();
  ...
  const handleCancel = () => {
    fetchData(); // Call this to update data after cancel action
  };
          ...
          <div className="min-h-screen max-w-6xl flex flex-col items-center container mx-auto pb-10 pt-5 px-4">
            ...
            <div className="bg-white shadow-lg p-6 w-full  border-2 border-black mx-auto">
              ...
              {role === "user" && !isCancelled && !isCompleted && (
                <CancelTransaction transactionId={data.id} fetchData={data} />
              )}
            </div>
          </div>
