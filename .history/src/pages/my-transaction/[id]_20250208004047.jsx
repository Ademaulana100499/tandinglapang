...
const DetailMyTransaction = ({ data, role }) => {
  ...
  const handleCancel = () => {
    // Perbaiki untuk memanggil fetchData hanya jika ada id
    if (data?.id) {
      fetchData();
    }
  };
...
export async function getServerSideProps(context) {
  const { role = null, token } = context.req.cookies; // Default to null if role is undefined
  ...
  try {
    ...
    return {
      props: {
        ...
        role, // Now 'role' will always have a valid value
      },
    };
  } catch (error) {
    ...
    return {
      props: {
        data: {},
        role: null, // Make sure 'role' is null if there was an error
      },
    };
  }
}
