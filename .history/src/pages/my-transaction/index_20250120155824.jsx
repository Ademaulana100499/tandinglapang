...
const MyTransaction = ({ transactions, page, totalPages }) => {
  return (
...
export async function getServerSideProps({ req, query }) {
  ...
  const search = query.search || ""; // Menangkap parameter search dari query string

  try {
    // Menyesuaikan URL API dengan parameter search, pagination, dan per_page
    ...
    const { result, total_pages } = res.data || { result: [], total_pages: 1 };

    // Pastikan totalPages selalu didefinisikan
    const totalPages = total_pages ?? 1;

    return { props: { transactions: result, page, totalPages, search } };
  } catch (error) {
    console.error("Error fetching transactions:", error);
    return { props: { transactions: [], page, totalPages: 1, search } };
  }
}
