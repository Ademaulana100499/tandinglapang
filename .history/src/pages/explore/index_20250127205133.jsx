...
const ActivityPage = ({ data, page }) => {
  ...
  return (
    <div id="explore " className="h-full md:h-screen bg-green-600">
      <Authorization>
        <Navbar />
        <section className="p-8  ">
          <SportCategoryDropdown />

          <div className="flex flex-col lg:flex-row h-fit md:h-screen w-full lg:h-[450px] bg-black w-full max-w-6xl mx-auto overflow-hidden">
            {data.length > 0 ? (
              data.map((item) => (
                <Panel
                  key={item.id}
