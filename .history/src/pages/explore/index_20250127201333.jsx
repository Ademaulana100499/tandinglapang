...
const ActivityPage = ({ data, page }) => {
  ...
  return (
    <div id="explore">
      <Authorization>
        <Navbar />
        <section className="p-4 h-full bg-green-600">
          <h1 className="text-2xl font-bold mb-4">Explore</h1>
          <SportCategoryDropdown />

          <div className="flex flex-col lg:flex-row h-screen lg:h-[450px] bg-black w-full max-w-6xl mx-auto overflow-hidden">
            {data.length > 0 ? (
              data.map((item) => (
                <Panel
                  key={item.id}
