...
const explorePage = () => {
    const SportActivities = ({ data }) => {
        ...
        const handleSearch = (e) => {
            e.preventDefault();
            router.push({ pathname: "/sport-activities", query: { search: searchTerm } });
          };
        ...
        <ul>
          {data.map((activity) => (
            <li key={activity.id}>
              ...
              <p>
                <strong>Category:</strong> {activity.sport_category.name} <br />
                <strong>Price:</strong> Rp {activity.price.toLocaleString()} (Discount: Rp {activity.price_discount.toLocaleString()}) <br />
                <strong>Slots:</strong> {activity.slot} <br />
                <strong>Location:</strong> {activity.address}, {activity.city.city_name} <br />
                <strong>Date:</strong> {activity.activity_date} <br />
                <strong>Time:</strong> {activity.start_time} - {activity.end_time} <br />
                <strong>Organizer:</strong> {activity.organizer.name} ({activity.organizer.email}) <br />
              </p>
              ...
              <ul>
                {activity.participants.map((participant) => (
                  <li key={participant.id}>{participant.user.name} ({participant.user.email})</li>
                ))}
              </ul>
            </li>
          ))}
