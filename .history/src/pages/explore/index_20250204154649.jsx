...
const ActivityPage = ({ data, page, role }) => {
          ...
          <section id="explore py-16 px-4">
            ...
            <div className="p-4">
              <div className="max-w-6xl mx-auto">
                {/* Show "Create Event" button only for admin role */}
                {role === "admin" && (
                  <button
                    onClick={(e) => {
                      e.preventDefault();
...
export async function getServerSideProps(context) {
  try {
    ...
    // Assuming that role is included in the response or token
    const role = context.req.cookies.role || "user"; // Default to user if not available

    return {
      props: { data: res.data.result.data || [], page: parseInt(page), role }, // Pass role to the component
    };
  } catch (error) {
    console.error("Error fetching activities:", error);
    return { props: { data: [], page: 1, role: "user" } };
  }
}
