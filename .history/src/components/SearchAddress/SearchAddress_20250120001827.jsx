...
const LocationDropdown = () => {
    ...
    const fetchProvinces = async () => {
      try {
        const response = await axios.get(
          `${process.env.NEXT_PUBLIC_API_URL}/provinces?is_paginate=true&per_page=5&page=${provincePage}`,
          {
            headers: {
  ...
  // Mengambil data kota berdasarkan ID provinsi yang dipilih
  useEffect(() => {
    if (selectedProvince) {
      const fetchCities = async () => {
        try {
          const response = await axios.get(
            `${process.env.NEXT_PUBLIC_API_URL}/location/cities/${selectedProvince}?is_paginate=true&per_page=5&page=${cityPage}`,
            {
              headers: {
                "Content-Type": "application/json",
                Authorization: `Bearer ${getCookie("token")}`,
