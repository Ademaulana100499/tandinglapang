...
const SportCategoryDropdown = ({ setSelectedCategory }) => {
  ...
  const { fetchCategories, loading, error } = useSportCategories();
  const [categories, setCategories] = useState([]);

  useEffect(() => {
    // Memanggil fetchCategories ketika komponen pertama kali di-render
    const getCategories = async () => {
      const result = await fetchCategories();
      setCategories(result); // Set categories setelah data diterima
    };
    getCategories();
  }, [fetchCategories]);
