...
import useSportCategories from "@/hooks/useSportCategories"; // Perbaiki import path
...
const SportCategoryDropdown = ({ setSelectedCategory }) => {
  ...
  const handleCategoryChange = (category) => {
    setSelectedCategory(category.id); // Mengirim id kategori ke parent
    setSelectedCategoryState(category.name); // Menyimpan nama kategori untuk ditampilkan
    setOpen(false);
  };
...
const Option = ({ text, Icon, setOpen, category, onClick }) => {
  const handleClick = () => {
    setOpen(false);
    onClick(category); // Menyampaikan kategori terpilih ke parent
  };
