                ...
                {paymentMethods.length > 0 ? (
                  paymentMethods.map((method) => (
                    <button
                      ...
                      onClick={() => {
                        onSelect({
                          id: method.id, // Kirim ID saja
                          image_url: method.image_url, // Kirim URL gambar saja
                        });
                        setIsOpen(false);
                      }}
                      className="w-full text-left py-3 px-4 border rounded-md mb-2 hover:bg-gray-100 transition">
                      <div className="flex justify-between">
                        <div>
                          <h4 className="font-semibold">{method.name}</h4>
                          <p>{method.virtual_account_number}</p>
                          <p>{method.virtual_account_name}</p>
                        </div>
                        <img src={method.image_url} alt="image" />
                      </div>
                    </button>
                  ))
                ) : (
