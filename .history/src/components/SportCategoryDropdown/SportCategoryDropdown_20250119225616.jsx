    ...
    const fetchCategories = async () => {
      try {
        const response = await axios.get(
          `${process.env.BASE_URL}/api/v1/sport-categories?is_paginate=true&per_page=5&page=${page}`
        );
      ...
      {loading ? (
        ...
        <>
          ...
          <div>
            ...
            <button
              onClick={() => handlePageChange(page + 1)}
              disabled={categories.length < 5} // Disable Next jika tidak ada halaman berikutnya
            >
              Next &raquo;
            </button>
          </div>
