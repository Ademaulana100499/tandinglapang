...
import useEditCategorySport from "@/hooks/useEditCategorySport";
import { BarLoader } from "../../Features/Loading";

export const EditCategorySport = ({ isOpen, setIsOpen, categoryId }) => {
  const { formData, setFormData, handleFormEdit, isLoading } =
    useEditCategorySport(setIsOpen, categoryId);
  return (
    <AnimatePresence>
      {isOpen && (
        <motion.div
            ...
            <form
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                <button
                  ...
                  {isLoading ? <BarLoader /> : "Ubah Kategori"}
                </button>
                <button
                  type="button"
                  onClick={() => setIsOpen(false)}
