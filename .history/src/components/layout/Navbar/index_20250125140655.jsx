...
const navigation = [
  { name: "Home", href: "#home", id: "home", current: false },
  { name: "Explore", href: "#explore", id: "explore", current: false },
  { name: "Category", href: "#category", id: "category", current: false },
...
export const Navbar = () => {
  ...
  // Menggunakan IntersectionObserver untuk mendeteksi bagian halaman
  useEffect(() => {
    const observer = new IntersectionObserver(
        ...
        entries.forEach((entry) => {
          const updatedNavItems = navItems.map((item) => {
            if (item.id === entry.target.id) {
              return { ...item, current: entry.isIntersecting };
            }
            return item;
          });
      ...
      { threshold: 0.5 } // Ubah nilai ini sesuai kebutuhan, 0.5 berarti 50% elemen harus terlihat
    );

    // Menambahkan observer untuk setiap elemen yang ada di navigation
