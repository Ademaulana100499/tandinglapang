...
export const OwnerSection = () => {
  ...
  useEffect(() => {
    console.log("Modal Dibuka, Role Sebelum:", formData.role);
    if (isOpen && !formData.role) {
      setFormData((prev) => ({ ...prev, role: "admin" }));
    }
    console.log("Role Setelah:", formData.role);
  }, [isOpen]);
  return (
      ...
      <section className="w-full  px-8 py-10 lg:py-24 sm:py-10 md:py-16  grid grid-cols-1 md:grid-cols-2 items-center gap-8 max-w-6xl mx-auto">
        <div>
          ...
          <button
            ...
            transition-all hover:bg-green-700 active:scale-95"> Daftar Sebagai
            Pemilik
          </button>
        </div>
        <ShuffleGrid />
